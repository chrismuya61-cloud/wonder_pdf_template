...
{
	if (TEMPLATE == 'wonder') {
		include_once module_dir_path(WPDF_TEMPLATE, 'libraries/Wonder_app_items_table.php');
		$class = new Wonder_app_items_table($transaction, $type, $for, $admin_preview);

		$class = hooks()->apply_filters('wonder_items_table_class', $class, $transaction, $type, $for, $admin_preview);

		if (!$class instanceof App_items_table_template) {
			show_error(get_class($class) . ' must be instance of "Wonder_app_items_template"');
		}
	} else if (TEMPLATE == 'gamma') {
...
if (!function_exists('set_qrcode')) {
	...
	function set_qrcode($data)
	{
		...
		/*$codeContents  =
			        ...
			        $codeContents .= $company_name."\n";
			        $codeContents .= 'Tel:'.$phone."\n";
		*/
