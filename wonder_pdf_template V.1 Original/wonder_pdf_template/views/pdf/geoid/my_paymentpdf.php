...
$company_info .= get_option('invoice_company_postal_code') . '<br />';
...
if (get_option('company_vat') != '') {
	$company_info .= '<br />' . _l('company_vat_number') . ': ' . get_option('company_vat');
}
...
$pdf->SetFontSize(11);
$pdf->Cell(80, 10, app_format_money($payment->amount, $payment->invoice_data->symbol), 0, 1, 'C', '1');
$pdf->Ln(5);
// The Table
$pdf->Ln(5);
$pdf->SetTextColor(0);
...
$tblhtml .= '<td>' . app_format_money($payment->invoice_data->total, $payment->invoice_data->symbol) . '</td>';
$tblhtml .= '<td>' . app_format_money($payment->amount, $payment->invoice_data->symbol) . '</td>';
$tblhtml .= '</tr>';
$tblhtml .= '</tbody>';
$tblhtml .= '</table>';
