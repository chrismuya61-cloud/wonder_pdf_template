...
$proposal_info .= get_option('company_state') . ' ';
$proposal_info .= get_option('invoice_company_postal_code') . '<br />';
...
if (get_option('company_vat') != '') {
	$proposal_info .= '<br />' . _l('company_vat_number') . ': ' . get_option('company_vat');
}
...
$project = '';
if (isset($proposal->project_id) && $proposal->project_id != '' && get_option('show_project_on_proposal') == 1) {
...
$items_data = pdf_get_table_items_and_taxes($proposal->items, 'proposal');
...
<tr>
    ...
    <td align="right" width="15%">' . app_format_money($proposal->subtotal, $proposal->symbol) . '</td>
</tr>';
if ($proposal->discount_percent != 0) {
    ...
    <tr>
        ...
        <td align="right" width="15%">-' . app_format_money($proposal->discount_total, $proposal->symbol) . '</td>
    </tr>';
}
foreach ($taxes as $tax) {
    ...
    <td align="right" width="15%">' . app_format_money($total, $proposal->symbol) . '</td>
</tr>';
}
...
if ((int) $proposal->adjustment != 0) {
    ...
    <td align="right" width="15%">' . app_format_money($proposal->adjustment, $proposal->symbol) . '</td>
</tr>';
}
$items_html .= '
<tr style="background-color:#f0f0f0;">
    <td align="right" width="85%"><strong>' . _l('estimate_total') . '</strong></td>
    <td align="right" width="15%">' . app_format_money($proposal->total, $proposal->symbol) . '</td>
</tr>';
$items_html .= '</table>';
...
$html = <<<EOF
<p style="font-size:18px; font-weight:bold;"># $number
<br /><span style="font-size:15px; font-weight:bold;">$proposal->subject</span>
</p>
$proposal_date
<br />
$open_till
