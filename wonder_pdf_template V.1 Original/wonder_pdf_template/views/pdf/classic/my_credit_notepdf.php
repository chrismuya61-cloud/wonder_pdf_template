...
<td style="text-align:right;"><h1 style="padding:0; margin:0; font-size:40px;">' . _l('credit_note_pdf_heading') . '</h1></td>
</tr>
...
$info_right_column .= '<b style="color:#4e4e4e; font-size: ' . ($font_size + 4) . 'px;"># ' . $credit_note_number . '</b>';

//dates
$info_right_column .= '<br><b style="color:#4e4e4e; font-size: ' . ($font_size + 4) . 'px;">' . _l('credit_note_date') . ' ' . _d($credit_note->date) . '</b>';
...
if (!empty($credit_note->reference_no)) {
  $info_right_column .= '<br><b style="color:#4e4e4e; font-size: ' . ($font_size + 4) . 'px;">' . _l('reference_no') . ' ' . $credit_note->reference_no . '</b>';
...
if ($credit_note->project_id != 0 && get_option('show_project_on_credit_note') == 1) {
  $info_right_column .= '<br><b style="color:#4e4e4e; font-size: ' . ($font_size + 4) . 'px;">' ._l('project') . ' ' . get_project_name_by_id($credit_note->project_id) . '</b>';
...
foreach ($pdf_custom_fields as $field) {
  ...
  $info_right_column .= '<br><b style="color:#4e4e4e; font-size: ' . ($font_size + 4) . 'px;">' .$field['name'] . ' ' . $value . '</b>';
}
...
$credit_note_info .= '<b style="color:black;">' . get_option('invoice_company_name') . '</b><br />';
$credit_note_info .= get_option('invoice_company_address') . '<br/>';
if (get_option('invoice_company_city') != '') {
  $credit_note_info .= get_option('invoice_company_city') . ', ';
}
$credit_note_info .= get_option('company_state') . ' ';
$credit_note_info .= get_option('invoice_company_postal_code') . '<br />';
$credit_note_info .= get_option('invoice_company_country_code') . '';

if (get_option('invoice_company_phonenumber') != '') {
  $credit_note_info .= '<br />' . get_option('invoice_company_phonenumber');
}
if (get_option('company_vat') != '') {
  $credit_note_info .= '<br />' . _l('company_vat_number') . ': ' . get_option('company_vat');
}
...
foreach ($custom_company_fields as $field) {
  $credit_note_info .= $field['label'] . ': ' . $field['value'] . '<br />';
}
$credit_note_info .= '</div>';
$info_bill_shipping .= $credit_note_info . '</td>';
...
<tr>
    <td width="55%"></td>
    <td align="right" width="30%" style="border:1px solid #ccc;" bgcolor="#f4f4f4"><strong>' . _l('credit_note_subtotal') . '</strong></td>
...
if ($credit_note->discount_percent != 0) {
  $tbltotal .= '
    <tr>
    <td width="55%"></td>
        <td align="right" width="30%" style="border:1px solid #ccc;" bgcolor="#f4f4f4"><strong>' . _l('credit_note_discount') . '(' . _format_number($credit_note->discount_percent, true) . '%)' . '</strong></td>
...
if ((int)$credit_note->adjustment != 0) {
  $tbltotal .= '<tr>
    <td width="55%"></td>
    <td align="right" width="30%" style="border:1px solid #ccc;" bgcolor="#f4f4f4"><strong>' . _l('credit_note_adjustment') . '</strong></td>
...
<tr>
    <td width="55%"></td>
    <td align="right" width="30%" style="border:1px solid #ccc;" bgcolor="#f4f4f4"><strong>' . _l('credit_note_total') . '</strong></td>
...
if ($credit_note->credits_used) {
  ...
  <tr>
      <td width="55%"></td>
      <td align="right" width="30%" style="border:1px solid #ccc;" bgcolor="#f4f4f4"><strong>' . _l('credits_used') . '</strong></td>
...
if ($credit_note->total_refunds) {
  ...
  <tr>
      <td width="55%"></td>
      <td align="right" width="30%" style="border:1px solid #ccc;" bgcolor="#f4f4f4"><strong>' . _l('refund') . '</strong></td>
...
$tbltotal .= '
  <tr>
      <td width="55%"></td>
      <td align="right" width="30%" style="border:1px solid #ccc;" bgcolor="#f4f4f4"><strong>' . _l('credits_remaining') . '</strong></td>
