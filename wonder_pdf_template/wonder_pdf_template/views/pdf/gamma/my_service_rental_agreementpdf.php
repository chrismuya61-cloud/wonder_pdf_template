...
<span style="font-size:' . ($font_size + 4) . 'px;"><b>Rental Date:</b> ' . (_d(isset($service_rental_agreement->start_date) ? $service_rental_agreement->start_date : '') ) . '
<br><b>Return Date:</b> ' . (_d(isset($service_rental_agreement->end_date) ? $service_rental_agreement->end_date : '') ) . '
<br><b>Received By:</b> ';

// Check for received_by
if (isset($service_rental_agreement->received_by) && $service_rental_agreement->received_by != 0) {
...
// Check for site_name
if (isset($service_rental_agreement->site_name) && $service_rental_agreement->site_name != null) {
    $info_right_column .= '<br><b>' . _l('site_name') . ':</b> ' . $service_rental_agreement->site_name;
}

// Check for field_operator
if (isset($service_rental_agreement->field_operator) && $service_rental_agreement->field_operator != null) {
...
if (isset($service_rental_agreement_client->show_primary_contact) && $service_rental_agreement_client->show_primary_contact == 1) {
...
$client_details .= isset($service_rental_agreement_client->company) ? $service_rental_agreement_client->company . '<br />' : '';
$client_details .= isset($service_rental_agreement_client->billing_street) ? $service_rental_agreement_client->billing_street . '<br />' : '';
$client_details .= !empty($service_rental_agreement_client->billing_city) ? $service_rental_agreement_client->billing_city : '';
$client_details .= !empty($service_rental_agreement_client->billing_state) ? ', ' . $service_rental_agreement_client->billing_state : '';
$billing_country = get_country_short_name($service_rental_agreement_client->billing_country);
$client_details .= !empty($billing_country) ? '<br />' . $billing_country : '';
$client_details .= !empty($service_rental_agreement_client->billing_zip) ? ', ' . $service_rental_agreement_client->billing_zip : '';
$client_details .= !empty($service_rental_agreement_client->vat) ? '<br />' . _l('invoice_vat') . ': ' . $service_rental_agreement_client->vat : '';
...
$info_service_details .= '</tbody>
      ...
      $info_service_details .= '<b style="font-size:12px; color:maroon;">SITE INFORMATION</b><br><br>
      <table border="1" cellpadding="4" class="table table-bordered table-striped dataTable">
          <thead>
              <tr>
          ...
          <tbody>
              <tr>
                  ...
                  <td style="text-align:center;">' . $field_report_info->site_name . '</td>
                  <td class="desc" style="text-align:center; font-weight:500;">' . _l('survey_type') . '</td>
                  <td style="text-align:center;">' . $field_report_info->survey_type . '</td>
              </tr>
              <tr>
                  <td class="desc" style="text-align:center; font-weight:500;">' . _l('survey_control_points_used') . '</td>
                  <td style="text-align:center;">' . $field_report_info->control_points_used . '</td>
                  <td class="desc" style="text-align:center; font-weight:500;">' . _l('Equipment Used') . '</td>
      ...
      <b style="font-size:12px; color:maroon;">REMARKS</b><br><br>
      <table border="1" cellpadding="4" class="table table-bordered table-striped dataTable">
          <thead>
              <tr>
          ...
          <tbody>
              ...
              <tr>
                ...
                // Add the report file section if it exists
    ...
    if (is_array($uploaded_files) && count($uploaded_files) > 0) {
        $info_service_details .= '<tr>
            <td class="desc">
                <label class="control-label" style="font-weight:500;">' . _l('Uploaded_file') . '</label>
                <ul>';
        foreach ($uploaded_files as $file) {
            $info_service_details .= '<li>
                <a href="' . base_url('modules/bizit_services_msl/uploads/reports/' . htmlspecialchars($file)) . '" target="_blank" class="btn btn-primary">
                    <i class="fa fa-file"></i> ' . htmlspecialchars($file) . '
                </a>
            </li>';
        }
        $info_service_details .= '</ul>
            </td>
        </tr>';
    } else {
        $info_service_details .= '<tr>
...
$remarks = '
    <b style="font-size: ' . ($font_size + 2) . 'px; font-weight:bold;">COMMENTS:</b><br>
    <table border="0" cellpadding="5">
      <tbody>
        <tr>
...
$terms_and_conditions = '
    <b style="font-size: ' . ($font_size + 2) . 'px; font-weight:bold; text-align:center;"><u>EQUIPMENT RENTAL TERMS AND CONDITIONS</u></b><br>
    <table border="0" cellpadding="5">
      <tbody>
        <tr>
        ...
        <tr>
         ...
         <td style="font-size: ' . ($font_size + 4) . 'px;">' . get_staff_full_name($service_rental_agreement->received_by) . '</td>
            ...
            <td style="vertical-align: middle; text-align:center; padding-top: 10px;">
                    ' . show_verification_qrcode(['verificationUrl' => site_url('service/field_report/view/' . $field_report_info->report_code)]) . '
            </td>
        </tr>
        </table>';
