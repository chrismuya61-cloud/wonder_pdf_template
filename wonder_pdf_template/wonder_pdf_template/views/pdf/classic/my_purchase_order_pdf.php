...
<td style="vertical-align: middle; font-size: '.($font_size+4).'px; border:1px solid ' . get_option('pdf_table_border_color') . ';"><b>'.strtoupper('Vendor Info').'</b></td>
<td style="vertical-align: middle; font-size: '.($font_size+4).'px; border:1px solid ' . get_option('pdf_table_border_color') . ';"><b>'.($purchase_order->inventory_related == 1 ? strtoupper(_l('ship_to')) : strtoupper(_l('purchaser_info'))).'</b></td>';
...
<thead>
<tr bgcolor="' . get_option('pdf_table_heading_color') . '" style="color:' . get_option('pdf_table_heading_text_color') . '; font-weight: bold;">
<td style="vertical-align: middle; font-size: '.($font_size+4).'px; border:1px solid ' . get_option('pdf_table_border_color') . ';"><b>'.($purchase_order->inventory_related == 1 ? strtoupper('Shipping Method') : strtoupper(_l('purchase_subject_short'))).'</b></td>
<td style="vertical-align: middle; font-size: '.($font_size+4).'px; border:1px solid ' . get_option('pdf_table_border_color') . ';"><b>' .strtoupper('Delivery Terms') . '</b></td>
<td style="vertical-align: middle; font-size: '.($font_size+4).'px; border:1px solid ' . get_option('pdf_table_border_color') . ';"><b>' .strtoupper('Delivery Date') . '</b></td>
</tr>
</thead>
<tbody>
<tr style="color:#424242;">
<td style="vertical-align: middle; font-size: '.($font_size+4).'px; border:1px solid ' . get_option('pdf_table_border_color') . ';">'.($purchase_order->inventory_related == 1 ? $purchase_order->shipping_mode : get_field_value('tblpurchasecategories', array('id'=>$purchase_order->subject), 'name')).'</td>';
$info_sales_person .= '<td style="vertical-align: middle; font-size: '.($font_size+4).'px; border:1px solid ' . get_option('pdf_table_border_color') . ';">Organized by Vendor</td><td style="vertical-align: middle; font-size: '.($font_size+4).'px; border:1px solid ' . get_option('pdf_table_border_color') . ';">On Order</td>';
...
<table border="0" bgcolor="#fff" cellspacing="0" cellpadding="5">
    <tr height="30" bgcolor="' . get_option('pdf_table_heading_color') . '" style="color:' . get_option('pdf_table_heading_text_color') . '; font-weight:bold;">
        ...
        <th width="8%" align="right" style="vertical-align: middle; font-size: '.($font_size+4).'px; border:1px solid ' . get_option('pdf_table_border_color') . ';">' . strtoupper($qty_heading) . '</th>
        <th width="14%" align="right" style="vertical-align: middle; font-size: '.($font_size+4).'px; border:1px solid ' . get_option('pdf_table_border_color') . ';">' . strtoupper(_l('invoice_table_rate_heading')) . '</th>';
        if($purchase_order->inventory_related == 0){
          $tblhtml .= '<th width="8%" align="right" style="vertical-align: middle; font-size: '.($font_size+4).'px; border:1px solid ' . get_option('pdf_table_border_color') . ';">' . strtoupper(_l('invoice_table_tax_heading')) . '</th>';
        }

        $tblhtml .= '<th width="15%" align="right" style="vertical-align: middle; font-size: '.($font_size+4).'px; border:1px solid ' . get_option('pdf_table_border_color') . ';">' . strtoupper(_l('invoice_table_amount_heading')) . '</th>
</tr>';
...
<tr>
    ...
    <td align="right" width="15%" style="border:1px solid #ccc;" bgcolor="#f4f4f4">' . format_money($subtotal, $purchase_order->symbol) . '</td>
</tr>'; 
$tbltotal .= $tax_rows;
}
...
<tr>
    ...
    <td align="right" width="15%" style="border:1px solid #ccc;" bgcolor="#f4f4f4">' . format_money($total, $purchase_order->symbol) . '</td>
</tr>';
